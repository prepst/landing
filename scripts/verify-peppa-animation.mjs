...
assert.match(homepage, /useMotionValue/)
assert.match(homepage, /useSpring/)
assert.match(homepage, /addEventListener\("pointermove"/)
assert.match(styles, /\.scroll-companion\s*\{[^}]*position:\s*fixed/)
assert.match(styles, /\.scroll-companion\s*\{[^}]*right:\s*28px/)
assert.match(styles, /\.scroll-companion\s*\{[^}]*top:\s*0/)
